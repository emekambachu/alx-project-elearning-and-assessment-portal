...
class YaedpSelectedProductService
{
    ...
    public function yaedpProcessFoodProductDetailByValuedChainName($valued_chain)
    {
        $valued_chain= $this->valuedChain->getValuedChainByName($valued_chain ?? 'Rubber')->first();

        return  YaedpProductDetail::where('yaedp_value_chain_id', $valued_chain->id ?? 8);
    }
    ...
    public function addProductByUserId($request, $id): array
    {
        ...
        // If type is a raw commodity submit it's parameters
        // Add to product Parameters
        if($input['type'] === 'Raw Commodities'){
