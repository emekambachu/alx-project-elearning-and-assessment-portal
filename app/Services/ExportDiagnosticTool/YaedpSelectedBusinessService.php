...
class YaedpSelectedBusinessService
{
    ...
    {
        return $this->yaedpBusinessDetail()->with('images', 'user', 'value_chain');
    }

    public function businessById($id){
        return $this->businessWithRelations()->firstOrFail($id);
    }

    public function getYaedpBusinessByUserId($id){
        return $this->businessWithRelations()->where('user_id', $id)->first();
    }
    ...
    public function updateBusinessByUserId($request, $businessId, $userId): array
    {
        $input = $request->all();
        $business = $this->businessById($businessId);
